  ...
  return (
    <section className={`${panel} radio-admin-page`}>
      ...
      <div className="mt-5 flex flex-wrap items-end justify-between gap-3">
        ...
        >
          {participants.isFetching ? <Busy label="새로고침 중" /> : <><RefreshCw className="size-4" />새로고침</>}
        </button>
      </div>

      {participants.isLoading && <Busy label="참가자 현황을 불러오는 중" />}
      {participants.error && !(participants.error instanceof ApiError && [401, 403].includes(participants.error.status)) && (
        <p role="alert" className="mt-6 rounded-lg bg-[#b5301f]/10 px-4 py-3 text-sm font-medium text-[#b5301f]">
          {errorMessage(participants.error) || '참가자 현황을 불러오지 못했습니다.'}
        </p>
      )}
                ...
                <table className="w-full min-w-[640px] border-collapse text-left text-sm">
                  ...
                  <tbody>
                    {data.participants.map((participant) => (
                      <tr key={participant.id} className="border-t border-[#ddd2bc]">
                        <td className="px-4 py-3 tabular">{formatTime(participant.createdAt)}</td>
                        <td className="px-4 py-3 font-semibold text-[#221f1d]">{participant.nickname}</td>
                        <td className="px-4 py-3 tabular">{formatPhone(participant.phone)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
