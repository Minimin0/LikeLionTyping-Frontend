      ...
      {data && (
        <div className="mt-7 space-y-7">
          <div className="grid grid-cols-2 gap-2 md:grid-cols-3">
            <Summary
              label="총 결제액"
              value={formatKrw(data.totalPaymentAmountKrw)}
            />
            <Summary
              label="결제 건수"
              value={`${data.totalPaymentCount.toLocaleString('ko-KR')}건`}
            />
            <Summary
              label="판매 이용권"
              value={`${data.totalPaidPassQuantity.toLocaleString('ko-KR')}회`}
                ...
                <ul className="space-y-2 md:hidden">
                  {data.payments.map((payment) => (
                    ...
                    >
                      ...
                      <p className="mt-3 text-sm text-[#7a675c] tabular">
                        {formatTime(payment.createdAt)}
                      </p>
                      <p className="mt-1 font-semibold text-[#221f1d]">
                        {formatKrw(payment.amountKrw)} · PAID +
                        {payment.quantity}회
                      </p>
                    </li>
                  ))}
...
function Summary({
  ...
  return (
    <div className={`rounded-lg bg-[#ece3d3]/60 p-4 ${className}`}>
